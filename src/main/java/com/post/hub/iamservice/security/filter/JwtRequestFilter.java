package com.post.hub.iamservice.security.filter;
    ...
    protected void doFilterInternal(
        ...
        if (authHeader.isPresent() && authHeader.get().startsWith(BEARER_PREFIX)) {
            ...
            try {
                ...
                Optional<String> usernameOpt = Optional.ofNullable(jwtTokenProvider.getUsername(jwt));
                Optional<String> userIdOpt = Optional.ofNullable(jwtTokenProvider.getUserId(jwt));
                if (usernameOpt.isPresent() && userIdOpt.isPresent()) {
